...
1. **Install RWANG into the project if missing** (makes it usable by any agent, not just Claude Code).
   - For each of `RWANG-MASTERPLAN.md`, `RWANG-CORE.md`, `RWANG-REVIEW.md`, `RWANG-OPTIMIZE.md`, `RWANG-VERSION.md`, `RWANG-INIT.md`: if the project root lacks it, copy it from this skill's directory. Never overwrite an existing copy — the project's copy is authoritative.
